...
function TechnicalMethodBadge({ service }: { service: Service }) {
  ...
  if (service.mikrotik_control_method === 'ip_firewall') return <div className="technical-badge"><span className="status-badge pending">IP/Firewall</span><small>{service.service_ip_address}</small></div>
  ...
  return <>
    ...
    <div className="technical-section full">
      ...
      <div className="method-options">
        ...
        <label><input type="radio" checked={form.mikrotik_control_method === 'ip_firewall'} onChange={() => setForm({ ...form, mikrotik_control_method: 'ip_firewall', pppoe_username: '', pppoe_password: '', pppoe_profile: '', simple_queue_name: '' })} /> IP/Firewall</label>
      </div>
    </div>
    ...
    {form.mikrotik_control_method === 'ip_firewall' && <label className="field full"><span>IP a cortar *</span><input required placeholder="192.168.0.120" value={form.service_ip_address} onChange={(event) => setForm({ ...form, service_ip_address: event.target.value, client_antenna_ip: form.client_antenna_ip || event.target.value })} /></label>}
