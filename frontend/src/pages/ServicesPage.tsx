import { ArrowRightLeft, Eye, PauseCircle, PlayCircle, Plus, RadioTower, RefreshCw, Router, Search, X } from 'lucide-react'
...
export function ServicesPage() {
  ...
  return (
    <section className="page-content services-page">
      ...
      {detail && <ServiceDetailModal service={detail} onClose={() => setDetail(null)} onRefresh={() => openDetail(detail)} onAction={(type) => { setDetail(null); setAction({ type, service: detail }) }} />}
...
function TechnicalConfigFields({ form, setForm, routers, plan }: { form: TechnicalForm; setForm: (form: TechnicalForm) => void; routers: MikrotikRouter[]; plan?: Plan | null }) {
  ...
  return <>
    ...
    {form.mikrotik_control_method === 'pppoe' && <><label className="field"><span>Usuario PPPoE *</span><input required maxLength={100} placeholder="Basado en la cedula" value={form.pppoe_username} onChange={(event) => setForm({ ...form, pppoe_username: event.target.value })} /></label><label className="field"><span>Contraseña PPPoE</span><input type="password" maxLength={255} autoComplete="new-password" placeholder="Solo escribir para crear o cambiar" value={form.pppoe_password} onChange={(event) => setForm({ ...form, pppoe_password: event.target.value })} /></label><PppoeProfileField routerId={form.mikrotik_router_id} value={form.pppoe_profile} onChange={(value) => setForm({ ...form, pppoe_profile: value })} /></>}
...
function PppoeProfileField({ routerId, value, onChange }: { routerId: string; value: string; onChange: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    api.get<{ data: string[] }>(`/mikrotik-routers/${routerId}/pppoe-profiles`)
      .then((response) => setProfiles(response.data.data))
      .catch(() => setError('No se pudieron leer los perfiles. Puedes escribirlo manualmente.'))
      .finally(() => setLoading(false))
  }, [routerId])

  const availableProfiles = routerId ? profiles : []
  return <label className="field"><span>Perfil PPPoE *</span>{availableProfiles.length > 0 ? <select required value={value} onChange={(event) => onChange(event.target.value)}><option value="">Selecciona un perfil</option>{availableProfiles.map((profile) => <option key={profile} value={profile}>{profile}</option>)}</select> : <input required maxLength={100} disabled={loading} placeholder={loading ? 'Consultando MikroTik...' : 'plan-30m'} value={value} onChange={(event) => onChange(event.target.value)} />}{error && <small>{error}</small>}</label>
}

function ServiceDetailModal({ service, onClose, onAction, onRefresh }: { service: Service; onClose: () => void; onAction: (type: Action['type']) => void; onRefresh: () => Promise<void> }) {
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const sync = async () => { setBusy(true); setError(''); try { await api.post(`/services/${service.id}/mikrotik/sync`); await onRefresh() } catch (requestError) { setError(getApiError(requestError)) } finally { setBusy(false) } }
  const retry = async (operation: MikrotikOperation) => { setBusy(true); setError(''); try { await api.post(`/services/${service.id}/mikrotik-operations/${operation.id}/retry`); await onRefresh() } catch (requestError) { setError(getApiError(requestError)) } finally { setBusy(false) } }
  return <div className="modal-backdrop"><section className="modal-card modal-wide"><header><div><span className="eyebrow">Servicio #{service.id}</span><h2>{service.client.full_name}</h2></div><button className="modal-close" onClick={onClose}><X /></button></header>{error && <div className="alert alert-error">{error}</div>}<div className="service-summary"><div><span>Plan actual</span><strong>{service.plan.name}</strong></div><div><span>Velocidad</span><strong>{service.plan.download_mbps}/{service.plan.upload_mbps} Mbps</strong></div><div><span>Zona</span><strong>{service.client.zone.name}</strong></div><div><span>Estado</span><span className={`status-badge ${service.status}`}>{service.status === 'active' ? 'Activo' : 'Suspendido'}</span></div></div><TechnicalSummary service={service} /><MikrotikOperationsSummary operations={service.mikrotik_operations ?? []} busy={busy} onRetry={retry} /><h3>Historial tecnico</h3><div className="timeline">{service.histories?.length ? service.histories.map((history) => <article key={history.id}><i /><div><strong>{history.description}</strong><time>{new Date(history.occurred_at).toLocaleString('es-BO')}</time></div></article>) : <p>Sin eventos registrados.</p>}</div><footer className="form-actions"><button className="button button-secondary" onClick={onClose}>Cerrar</button>{service.mikrotik_control_method !== 'manual' && <button className="button button-secondary" disabled={busy} onClick={() => void sync()}><RefreshCw size={16} /> Sincronizar</button>}<button className="button button-secondary" onClick={() => onAction('technical')}><Router size={16} /> MikroTik</button><button className="button button-secondary" onClick={() => onAction('plan')}><ArrowRightLeft size={16} /> Cambiar plan</button>{service.status === 'active' ? <button className="button button-danger" onClick={() => onAction('suspend')}>Suspender</button> : <button className="button button-primary button-fit" onClick={() => onAction('reactivate')}>Reactivar</button>}</footer></section></div>
}

function MikrotikOperationsSummary({ operations, busy, onRetry }: { operations: MikrotikOperation[]; busy: boolean; onRetry: (operation: MikrotikOperation) => Promise<void> }) {
  const actionLabels: Record<MikrotikOperation['action'], string> = { create_access: 'Crear acceso', suspend: 'Suspender', reactivate: 'Reactivar', change_plan: 'Cambiar plan' }
  const statusLabels: Record<MikrotikOperation['status'], string> = { pending: 'Pendiente', processing: 'Procesando', synced: 'Sincronizado', failed: 'Fallido' }

  return <section className="sync-list"><h3>Operaciones MikroTik</h3>{operations.length ? operations.map((operation) => <article key={operation.id}><div><strong>{actionLabels[operation.action]}</strong><span>{operation.router?.name ?? 'Sin router'} · {operation.attempts} intento{operation.attempts === 1 ? '' : 's'}</span>{operation.last_error && <small>{operation.last_error}</small>}</div><span className={`status-badge ${operation.status}`}>{statusLabels[operation.status]}</span>{operation.status === 'failed' && <button className="button button-secondary button-fit" disabled={busy} onClick={() => void onRetry(operation)}><RefreshCw size={15} /> Reintentar</button>}</article>) : <p>Sin operaciones pendientes.</p>}</section>
}
